...
def draw(proto):
    ...
    plt.plot(x, old_ave, label='average w/o acceleration', color='b')
    plt.fill_between(x, old_min, old_max, color='b', alpha=0.2, label='min-max w/o acceleration')  # 填充最大值和最小值之间的区域

    plt.plot(x, new_ave, label='average with acceleration', color='g')
    ...
    plt.ylabel('Execution Time (s)', fontweight='bold', labelpad= 5)
